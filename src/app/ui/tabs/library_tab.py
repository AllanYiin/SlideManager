...
class LibraryTab(QWidget):
    ...
    def _start_index(self, files: List[Dict[str, Any]], update_text: bool, update_image: bool) -> None:
        ...
        log.info(
            "[INDEX_FLOW][START] prepared file_paths total=%d scope_only=%s",
            len(file_paths),
            self._index_status_payload.get("scope_only"),
        )
        ...
        for root in library_roots:
            ...
            for path in file_paths:
                try:
                    ...
                    log.exception("[INDEX_FLOW][START] scope map failed path=%s root=%s", path, root)
                    continue
            if scoped:
                roots_with_files.append(root)
                files_by_root[root] = scoped
            log.info(
                "[INDEX_FLOW][START] scope map root=%s files=%d",
                root,
                len(scoped),
            )
