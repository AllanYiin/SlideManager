...
class JobManager:
    ...
    async def _plan_jobs(
        ...
        if options.file_paths:
            logger.info(
                "job planning uses file_paths: total=%d root=%s",
                len(options.file_paths),
                root,
            )
            scans = scan_specific_files(options.file_paths)
        else:
            logger.info("job planning scans root: %s", root)
            scans = scan_files_under(root)
        logger.info("job planning scan result: files=%d job_id=%s", len(scans), job_id)
